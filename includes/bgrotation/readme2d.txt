...
Parameters:
s - the shape whose center is to be returned
Return Value:
A vector pointing to the center of the given shape.
...
Return Value:
A string that describes the given shape, or "null" if a null handle is provided.
...
Remarks:
Unlike translate, rotate, and scale, the set_ methods modify the original shape.

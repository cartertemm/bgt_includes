...
if hasattr(sys, 'frozen'):
	path = os.path.join(os.path.abspath(os.path.dirname(sys.executable)), 'camlorn_audio')
	al_soft = cdll.LoadLibrary(os.path.join(path, 'soft_oal.dll'))
	libsndfile = cdll.LoadLibrary(os.path.join(path, 'libsndfile-1.dll'))
	ca_module = cdll.LoadLibrary(os.path.join(path, 'camlorn_audio_c.dll'))
else:
